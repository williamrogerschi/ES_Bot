...
class GridStrategy:
    ...
    async def on_new_bar(self, bar: Dict):
        ...
        if self.grid_levels:
            ...
            # Determine next entry direction and price
            trend = self.confirmed_trend
            if trend in [TrendState.STRONG_BULLISH, TrendState.MODERATE_BULLISH]:
                # Bullish: looking to go long at support (below current price)
                next_levels = [l for l in self.grid_levels if l <= self.last_price]
                if next_levels:
                    next_price = max(next_levels)  # Closest level below
                    next_str = f"Next: LONG @ {next_price:.2f}"
                else:
                    next_str = ""
            elif trend in [TrendState.STRONG_BEARISH, TrendState.MODERATE_BEARISH]:
                # Bearish: looking to go short at resistance (above current price)
                next_levels = [l for l in self.grid_levels if l >= self.last_price]
                if next_levels:
                    next_price = min(next_levels)  # Closest level above
                    next_str = f"Next: SHORT @ {next_price:.2f}"
                else:
                    next_str = ""
            else:
                # Sideways: closest level in either direction
                if above and below:
                    above_dist = float(above[0]) - self.last_price if above else float('inf')
                    below_dist = self.last_price - float(below[0]) if below else float('inf')
                    if above_dist <= below_dist:
                        next_str = f"Next: SHORT @ {above[0]}"
                    else:
                        next_str = f"Next: LONG @ {below[0]}"
                elif above:
                    next_str = f"Next: SHORT @ {above[0]}"
                elif below:
                    next_str = f"Next: LONG @ {below[0]}"
            ...
            grid_line = f"     Grid ↑: {above}  Grid ↓: {below}"
            if next_str:
                grid_line += f" | {next_str}"
            print(grid_line)